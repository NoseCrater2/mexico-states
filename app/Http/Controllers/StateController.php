...
class StateController extends Controller {
    ...
    private $cache;
    function __construct( CacheService $cache) {
        $this->cache = $cache;
    }

    function index() {

        $data = $this->cache->remember('dashboard-data', fn() => [
    ...
    function getStates(Request $request) {
        ...
        ksort($queryParams);
        $cacheKey = 'states:' . md5(json_encode($queryParams));
        ...
        $states = $this->cache->remember($cacheKey, function () use ($sort, $direction, $search) {
           return  State::query()
    ...
    function showState(Request $request, string $id) {
        ...
        $municipalities = app(MunicipalityService::class)($id, $this->cache);
        if($search){
